...
其实，自身属性是所有属性的子集，按理说，迭代所有属性就可以分析出自身属性。假设我们有这样的一个函数：
...
getAllProperties(o).forEach(property => {
  if (property.isSelf) {
    console.log(`自身属性：${property.name}`)
  } else {
    console.log(`来自原型链的属性：${property.name}`)
  }
})
```

但是，JavaScript没有这样的API。在JavaScript中，迭代对象的属性，都是拿到一个 `string` 类型的属性名变量，除非
